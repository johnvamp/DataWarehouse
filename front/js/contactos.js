...
function traerContactos() {
	const urlContactos = "http://localhost:3500/contactos";
	let requestOptions = {
		method: "GET",
		headers: myHeaders,
	};
	fetch(urlContactos, requestOptions)
		...
		.then((json) => {
			...
			for (x = 0; x < json.length; x++) {
				let contLimpio = {
					id_contacto: json[x].id_contacto,
					p_nombre: json[x].p_nombre,
					apellido: json[x].apellido,
					cargo: json[x].cargo,
					email: json[x].email,
					...
					interes: json[x].interes,
					telefono_compania: json[x].telefono_compania,
					telefono_personal: json[x].telefono_personal,
					whatsapp_compania: json[x].whatsapp_compania,
					whatsapp_personal: json[x].whatsapp_personal,
					instagram_compania: json[x].instagram_compania,
					instagram_personal: json[x].instagram_personal,
					facebook_compania: json[x].facebook_compania,
					facebook_personal: json[x].facebook_personal,
					linkedin_compania: json[x].linkedin_compania,
					linkedin_personal: json[x].linkedin_personal,
				};
...
function datosContactos(json) {
	for (i = 0; i < json.length; i++) {
		...
		contactos.setAttribute("id", "contacto" + json[i].id_contacto);
		...
		let contenedorBarraInteres = document.createElement("div");
		let barraProgreso = document.createElement("meter");
		contenedorBarraInteres.appendChild(barraProgreso);
		contenedorBarraInteres.appendChild(interes);
		...
		barraProgreso.low = "30";
		barraProgreso.high = "75";
		barraProgreso.optimum = "100";
		barraProgreso.value = json[i].interes;
...
function cabeceraContactos(arrayContactos) {
	...
	lupa.setAttribute("class", "fas fa-search botonLupa");
...
function busqueda(arrayContactos) {
	...
	let encontrado = arrayContactos.filter(
		(a) =>
			a.p_nombre.toLowerCase().includes(palabra.toLowerCase()) == true ||
			a.apellido.toLowerCase().includes(palabra.toLowerCase()) == true ||
			a.cargo.toLowerCase().includes(palabra.toLowerCase()) == true ||
...
function postContact() {
	...
	let newContact = {
		...
		telefono_compania: ctelefono.value,
		telefono_personal: preTel,
		whatsapp_compania: cwhatsapp.value,
		whatsapp_personal: preWha,
		instagram_compania: cinstagram.value,
		instagram_personal: preIns,
		facebook_compania: cfacebook.value,
		facebook_personal: preFac,
		linkedin_compania: clinkedin.value,
		linkedin_personal: preLin,
	};
...
function formularioContacto(data) {
	...
	lblForm4.innerHTML = "Correo electrónico*";
	lblForm5.setAttribute("for", "id_company");
	lblForm5.innerHTML = "Compañía*";
	...
	opcion0.innerHTML = "Seleccione una compañía...";
	...
	lblForm04.innerHTML = "Dirección:";
	lblForm05.setAttribute("for", "cinteres");
	lblForm05.innerHTML = "Interés:";
	...
	opcionReg.innerHTML = "Seleccione una Región...";
	...
	selectReg.addEventListener("change", function () {
		...
		opcionPai.innerHTML = "Seleccione un País...";
	...
	if (data) {
		...
		aviso.innerHTML =
			"Sálo se aplicarán cambios cuando exista iteracción del usuario sobre el campo, de lo contrario se mantendrá la misma información.";
		inputForm1.setAttribute("value", data.p_nombre);
		...
		selectForm5.setAttribute("value", data.id_compania);
		...
		inputForm06.setAttribute("value", data.telefono_compania);
		selectForm06.setAttribute("value", data.telefono_personal);
		inputForm07.setAttribute("value", data.whatsapp_compania);
		selectForm07.setAttribute("value", data.whatsapp_personal);
		inputForm08.setAttribute("value", data.instagram_compania);
		selectForm08.setAttribute("value", data.instagram_personal);
		inputForm09.setAttribute("value", data.facebook_compania);
		selectForm09.setAttribute("value", data.facebook_personal);
		inputForm010.setAttribute("value", data.linkedin_compania);
		selectForm010.setAttribute("value", data.linkedin_personal);
...
function eliminarContacto(id_contact) {
	...
	if (confirmacion == true) {
		deleteContacto(id_contact);
	}
}

function patchContact(id_contacto) {
	let companyContact = document.getElementById("id_company").value;
	let ciudadContact = document.getElementById("cciudad").value;
	let interesContact = document.getElementById("cinteres").value;
	let preTel = document.getElementById("ctel").value;
	...
	const urlContactos = `http://localhost:3500/contactos/${id_contacto}`;
	fetch(urlContactos, requestOptions)
		.then((res) => res.json())
		.then((json) => {
			console.log(json);
...
function seleccionEliminar() {
	...
	multiDelete.addEventListener("click", function () {
		...
		if (confirmacion == true) {
			for (i = 0; i < arrayEliminar.length; i++) {
				deleteContactos(arrayEliminar[i]);
			}
			alert("Contacto eliminado exitosamente");
			cargarContactos();
		}
...
function deleteContactos(id_contact) {
	let requestOptions = {
		method: "DELETE",
		headers: myHeaders,
	};
